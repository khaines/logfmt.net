    ...
    public class ExtensionLoggingTests
    {
            ...
            var state = new List<KeyValuePair<string, object>>
      {
        ...
        new KeyValuePair<string, object>("dupe", "test message4"),
            ...
            var reader = new StreamReader(outputStream);
            var output = reader.ReadLine();
            Assert.EndsWith("level=warn foo=bar msg=\"test message\" dupe=\"test message4\" event_id=1 event_name=1", output);
        }

        private ExtensionLoggerConfiguration GetConfiguration() => new ExtensionLoggerConfiguration() { LogLevel = new Dictionary<string, LogLevel> { { "Default", LogLevel.Information } } };
    }
}